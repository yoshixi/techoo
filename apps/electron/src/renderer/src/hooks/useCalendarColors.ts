 ...
 * user-chosen overrides in localStorage.
 */
export function useCalendarColors(
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(CALENDAR_COLORS_STORAGE_KEY)
      if (raw) {
  ...
  useEffect(() => {
    ...
    setStored((prev) => {
      ...
      localStorage.setItem(CALENDAR_COLORS_STORAGE_KEY, JSON.stringify(merged))
      return merged
    })
  ...
  const setCalendarColor = useCallback((calendarId: string, color: string) => {
    ...
    setStored((prev) => {
      ...
      localStorage.setItem(CALENDAR_COLORS_STORAGE_KEY, JSON.stringify(next))
      return next
    })
  }, [])

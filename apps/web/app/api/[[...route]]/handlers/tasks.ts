...
export const listTasksHandler: RouteHandler<typeof listTasksRoute> = async (c) => {
  try {
    ...
    const { completed, hasActiveTimer, sortBy, order, tags } = c.req.valid('query')

    const tasks = await getAllTasks(db, defaultUser.id.toString(), { completed, hasActiveTimer, sortBy, order, tags })

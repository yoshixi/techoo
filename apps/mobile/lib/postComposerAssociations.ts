...
export function parseAssociationsParam(value?: string | string[]): PostComposerAssociations | null {
  const raw = Array.isArray(value) ? value[0] : value;
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as PostComposerAssociations;
    return {
...
export async function createPostWithAssociations(
  body: string,
  associations: PostComposerAssociations
): Promise<void> {
